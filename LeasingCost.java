public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        int fragmentIndex = description.indexOf("name:");
        int dataIndex = fragmentIndex + 5;
        String newString = description.substring(dataIndex);
        int indexEnd = newString.indexOf(';');
        String nameData = newString.substring(0, indexEnd);

        fragmentIndex = description.indexOf("type:");
        dataIndex = fragmentIndex + 5;
        newString = description.substring(dataIndex);
        indexEnd = newString.indexOf(';');
        String typeData = newString.substring(0, indexEnd);

        fragmentIndex = description.indexOf("mile/unit:");
        dataIndex = fragmentIndex + 10;
        newString = description.substring(dataIndex);
        indexEnd = newString.indexOf(';');
        String unitData = newString.substring(0, indexEnd);
        double doubleUnitData = Double.parseDouble(unitData);

        Fuel fuel; 

        if (typeData.equals("electric")) {
            fragmentIndex = description.indexOf("charger:");
            dataIndex = fragmentIndex + 8;
            newString = description.substring(dataIndex);
            indexEnd = newString.indexOf(';');
            String chargerData = newString.substring(0, indexEnd);
            double doubleChargerData = Double.parseDouble(chargerData);
            fuel = new Fuel(doubleUnitData, doubleChargerData);
        }

        else {
            fuel = new Fuel(doubleUnitData);
        }

        fragmentIndex = description.indexOf("due:");
        dataIndex = fragmentIndex + 4;
        newString = description.substring(dataIndex);
        indexEnd = newString.indexOf(';');
        String dueData = newString.substring(0, indexEnd);
        double doubleDueData = Double.parseDouble(dueData);

        fragmentIndex = description.indexOf("length:");
        dataIndex = fragmentIndex + 7;
        newString = description.substring(dataIndex);
        indexEnd = newString.indexOf(';');
        String lengthData = newString.substring(0, indexEnd);
        int intLengthData = Integer.parseInt(lengthData);

        fragmentIndex = description.indexOf("monthly:");
        dataIndex = fragmentIndex + 8;
        newString = description.substring(dataIndex);
        indexEnd = newString.indexOf(';');
        String monthlyData = newString.substring(0, indexEnd);
        double doubleMonthlyData = Double.parseDouble(monthlyData);

        fragmentIndex = description.indexOf("allowance:");
        dataIndex = fragmentIndex + 10;
        newString = description.substring(dataIndex);
        indexEnd = newString.indexOf(';');
        String mileageAllowanceData = newString.substring(0, indexEnd);
        int intMileageAllowanceData = Integer.parseInt(mileageAllowanceData);
    
        Lease lease = new Lease(doubleDueData,intLengthData, doubleMonthlyData, intMileageAllowanceData);
        Vehicle v = new Vehicle(nameData, fuel, lease);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
	    for (Vehicle vehicle: vehicles) {
            ...
            double co2Emission = computeCO2(numberOfMonths, usage, mileageAllowance, CO2PerUnit);
            vehicle.setCO2Emission(co2Emission);
            double fuelCost = computeFuelCost(numberOfMonths, usage, mileageAllowance, fuelPrice);
            vehicle.setFuelCost(fuelCost);
            double leaseCost = computeLeaseCost(dueAtSigning, numberOfMonths, monthlyCost);

            if (fuel.getType() == 1) {
                vehicle.setTotalCost(fuelCost + leaseCost);
            }
            else {
                double chargerCost = fuel.getCharger();
                vehicle.setTotalCost(chargerCost + fuelCost + leaseCost);
            }
